...
def setup():
    ...
    elif opt.args.name == 'citeseer':
        opt.args.n_clusters = 6
        opt.args.n_input = 100
        opt.args.alpha_value = 0.2
        opt.args.lambda_value = 1
        opt.args.gamma_value = 1e3
        opt.args.lr = 1e-5
    ...
    opt.args.device = torch.device(f"cuda:{opt.args.cuda}" if opt.args.cuda >= 0 else "cpu")
    print("------------------------------")
...
def model_init(model, X, y, A_norm):
    ...
    with torch.no_grad():
        _, _, _, sim, _, _, _, Z, _, _ = model(X, A_norm, X, A_norm, A_norm)

    # calculate cluster centers
    acc, nmi, ari, f1, centers, _ = clustering(Z, y)

    return sim, centers


# the reconstruction function from DFCN
def reconstruction_loss(X, A_norm, X_hat, Z_hat_all, A_hat_all):
    """
    ...
    Args:
        ...
        Z_hat: the reconstructed Z, Z_view1, Z_view2
        A_hat: the reconstructed A, A_view1, A_view2
    Returns: the reconstruction loss
    """
    loss_ae = F.mse_loss(X_hat, X)
    loss_w = F.mse_loss(Z_hat_all[0], torch.spmm(A_norm, X))
    loss_a = F.mse_loss(A_hat_all[0], A_norm.to_dense())
    # loss_w1 = F.mse_loss(Z_hat_all[1], torch.spmm(A_norm, X))
    # loss_a1 = F.mse_loss(A_hat_all[1], A_norm.to_dense())
    # loss_w2 = F.mse_loss(Z_hat_all[2], torch.spmm(A_norm, X))
    # loss_a2 = F.mse_loss(A_hat_all[2], A_norm.to_dense())
    loss_igae = loss_w + 0.1 * loss_a
    # loss_igae1 = loss_w1 + 0.1 * loss_a1
    # loss_igae2 = loss_w2 + 0.1 * loss_a2
    loss_rec = loss_ae + loss_igae # + loss_igae1 + loss_igae2
    return loss_rec
...
def improved_correlation_loss(S, T):
    return S[T != 0].add(-1).pow(2).mean() + S[T == 0].pow(2).mean()


def dicr_loss(Z_ae, Z_igae, AZ, Z, T, Z_emb):
    """
    ...
    Args:
        ...
        Z_emb: the final embedding of nodes
        Z_emb_feature: final embedding cluster-level
    Returns:
        L_{DICR}
    """
    ...
    # S_N_node = cross_correlation(Z_emb, Z_emb)
    # L_N_node = improved_correlation_loss(S_N_node, T)
...
def clustering(Z, y):
    ...
    """
    cluster_id, cluster_centers = kmeans(Z, num_clusters=opt.args.n_clusters, device=opt.args.device)
    acc, nmi, ari, f1 = eva(y, cluster_id.detach().numpy(), show_details=opt.args.show_training_details)
    return acc, nmi, ari, f1, cluster_centers.detach().numpy(), cluster_id
...
def high_confidence(Z, z_centers, Z_labels=None, threshold=0.1):
    ...
    values = torch.topk(distance.values, int(Z.shape[0] * 0.5)).values
    hc = 0
    for i in torch.unique(dis_labels):
        ...
        hc += mask_i.t() @ mask_i
    # false_idx = distance.values>values[-1]
    # hc[:, false_idx] = -1
