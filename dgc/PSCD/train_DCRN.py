...
def train(model, X, y, A, A_norm, Ad):
    ...
    for epoch in tqdm.tqdm(range(opt.args.epoch)):
        ...
        acc, nmi, ari, f1, _, z_labels = clustering(Z, y)
        if acc > opt.args.acc:
            opt.args.acc = acc
            opt.args.nmi = nmi
            opt.args.ari = ari
    ...
    torch.save(final_emb, f'DCRN_{opt.args.name}_emb.pt')
    torch.save(final_label, f'DCRN_{opt.args.name}_label.pt')
