...
class PSCD(nn.Module):
    def __init__(self, n_node=None):
        super(PSCD, self).__init__()
    ...
    def forward(self, X_tilde1, Ad, X_tilde2, Ak, A_norm):
        # node embedding encoded by AE
        Z_ae1 = self.ae.encoder(X_tilde1)
        Z_ae2 = self.ae.encoder(X_tilde2)

        # node embedding encoded by IGAE
        Z_igae1, A_igae1, AZ_1, Z_1 = self.gae.encoder(X_tilde1, Ad)
        Z_igae2, A_igae2, AZ_2, Z_2 = self.gae.encoder(X_tilde2, Ak)
        ...
        Z_hat1, Z_adj_hat1, _, _ = self.gae.decoder(Z, Ad)
        Z_hat2, Z_adj_hat2, _, _ = self.gae.decoder(Z, Ak)
        sim = (A_igae1 + A_igae2) / 2
        A_hat = sim + Z_adj_hat
        A_hat1 = A_igae1 + Z_adj_hat1
        A_hat2 = A_igae2 + Z_adj_hat2
        Z_hat_all = [Z_hat, Z_hat1, Z_hat2]
        A_hat_all = [A_hat, A_hat1, A_hat2]
        ...
        return X_hat, Z_hat_all, A_hat_all, sim, Z_ae_all, Z_gae_all, Q, Z, AZ_all, Z_all
