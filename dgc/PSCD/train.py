...
def train(model, X, y, A, A_norm, Ad):
    ...
    for epoch in tqdm.tqdm(range(opt.args.epoch)):
        ...
        # input & output
        X_hat, Z_hat_all, A_hat_all, _, Z_ae_all, Z_gae_all, Q, Z, AZ_all, Z_all = model(X_tilde1, Ad, X_tilde2, Ak, Ad)
        # X_hat, Z_hat_all, A_hat_all, _, Z_ae_all, Z_gae_all, Q, Z, AZ_all, Z_all = model(X, A_norm, X, Ak, Ad)
        ...
        L_DICR = dicr_loss(Z_ae_all, Z_gae_all, AZ_all, Z_all, T, Z)
        L_REC = reconstruction_loss(X, A_norm, X_hat, Z_hat_all, A_hat_all)
        ...
        if acc > opt.args.acc:
            ...
            opt.args.f1 = f1
            final_emb = Z
            final_label = z_labels

    torch.save(final_emb, f'PSCD_{opt.args.name}_emb.pt')
    torch.save(final_label, f'PSCD_{opt.args.name}_label.pt')
