...
class SGCN_encoder(nn.Module):
    def __init__(self, in_dim, out_dim):
        super().__init__()
        self.gcn_1hop = GNNLayer(in_dim, out_dim)
        self.gcn_2hop = GNNLayer(in_dim, out_dim)
        self.gcn_3hop = GNNLayer(in_dim, out_dim)
        self.project = nn.Linear(out_dim * 3, out_dim)
        self.s = nn.Sigmoid()
    ...
    def forward(self, z, adj):
        z_1hop, az_1 = self.gcn_1hop(z, adj[0], active=True)
        z_2hop, az_2 = self.gcn_2hop(z, adj[1], active=True)
        z_3hop, az_3 = self.gcn_3hop(z, adj[2], active=True)
        z_igae = self.project(torch.cat([z_1hop, z_2hop, z_3hop], dim=1))
        z_igae_adj = self.s(torch.mm(z_igae, z_igae.t()))
        return z_igae, z_igae_adj, [az_1, az_2, az_3], [z_1hop, z_2hop, z_3hop]


class SGCN_decoder(nn.Module):
    def __init__(self, in_dim, out_dim):
        super().__init__()
        self.gcn_1hop = GNNLayer(in_dim, out_dim)
        self.gcn_2hop = GNNLayer(in_dim, out_dim)
    ...
    def forward(self, z, adj):
        z_1hop, az_1 = self.gcn_1hop(z, adj[0])
        z_2hop, az_2 = self.gcn_2hop(z, adj[1])
        z_3hop, az_3 = self.gcn_3hop(z, adj[2])
        z_hat = self.project(torch.cat([z_1hop, z_2hop, z_3hop], dim=1))
        z_hat_adj = self.s(torch.mm(z_hat, z_hat.t()))
        return z_hat, z_hat_adj, [az_1, az_2, az_3], [z_1hop, z_2hop, z_3hop]
...
class SGCN(nn.Module):
    def __init__(self, in_feature, out_feature):
        super().__init__()
        self.encoder = SGCN_encoder(in_feature, out_feature)
        self.decoder = SGCN_decoder(out_feature, in_feature)
...
class FFDReadout(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_dim):
        super().__init__()
        self.linear = nn.Linear(in_dim * hidden_dim, out_dim * hidden_dim)
        self.hidden_dim = hidden_dim
        nn.init.xavier_normal_(self.linear.weight)

    def forward(self, z):
        z_c = self.linear(z.reshape(1, -1)).reshape(-1, self.hidden_dim)
        return z_c
...
class DCRN(nn.Module):
    def __init__(self, n_node=None):
        ...
        # SIGN gae
        # self.sgae = SGCN(opt.args.n_input, opt.args.gae_n_enc_3)
        ...
        self.project = nn.Linear(2 * opt.args.n_z, opt.args.n_z)
        nn.init.xavier_normal_(self.project.weight)
        ...
        # self.R = FFDReadout(n_node, opt.args.n_clusters, opt.args.n_z)

        # sigmoid for igae decode
        # self.s = nn.Sigmoid()
    ...
    def forward(self, X_tilde1, Am, X_tilde2, Ad, A_norm):
        ...
        # Z_igae1, A_igae1, AZ_1, Z_1 = self.sgae.encoder(X_tilde1, Am_all)
        # Z_igae2, A_igae2, AZ_2, Z_2 = self.sgae.encoder(X_tilde2, Ad_all)
        ...
        # attention combination of two views
        # Z_ae = self.ae_a * Z_ae1 + (1 - self.ae_a) * Z_ae2
        # Z_igae = self.igae_a * Z_igae1 + (1 - self.igae_a) * Z_igae2
        ...
        # S = F.softmax(S, dim=1)
        # Z_g = torch.mm(S, Z_l)
        Z_g_1 = torch.spmm(A_norm, Z_l)
        Z_g_2 = torch.spmm(A_norm, Z_g_1)
        Z_g = self.project(torch.cat([Z_g_1, Z_g_2], dim=1))
        ...
        # IGAE decoding
        Z_hat, Z_adj_hat, AZ_de, Z_de = self.gae.decoder(Z, A_norm)
