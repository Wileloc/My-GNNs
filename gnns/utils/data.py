...
def load_data(name, device='cpu', add_reverse_edge=True, reverse_self=True):
    ...
    :param name: str 数据集名称 acm, dblp, ogbn-mag, oag-venue, oag-field
...
def rand_node_feat(g):
    for ntype in g.ntypes:
        if 'feat' not in g.nodes[ntype].data:
            g.nodes[ntype].data['feat'] = torch.rand((g.num_nodes(ntype), 128), device=g.device)
