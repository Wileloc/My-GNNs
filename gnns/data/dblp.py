...
from gnns.utils import split_idx
...
class DBLPFourAreaDataset(DGLDataset):
    ...
    * label: tensor(20)，类别为0~3
    ...
    def _add_ndata(self):
        ...
        # self.g.multi_update_all({'ap': (fn.copy_u('feat', 'm'), fn.mean('m', 'feat'))}, 'sum')
        # 论文特征为作者特征的平均
        self.g.update_all(fn.copy_u('feat', 'm'), fn.mean('m', 'feat'), etype='ap')
        #其他顶点属性为one-hot向量
        self.g.nodes['term'].data['feat'] = torch.eye(self.g.num_nodes('term'))
        self.g.nodes['conf'].data['feat'] = torch.eye(self.g.num_nodes('conf'))
